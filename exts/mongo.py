...
class MongDb(object):
    ...
    def __init__(self, dbhost, dbport, dbname, dbuser=None, dbpass=None, log=None):
        try:
            self.log = log

            self.conn = pymongo.MongoClient(dbhost, dbport,
                                            connectTimeoutMS=3 * 60 * 1000,
                                            serverSelectionTimeoutMS=3 * 60 * 1000,
                                            maxPoolSize=600)
    ...
    def update_save(self, table, data):
        ...
        if _id is None:
            return
        result = self.update(table, {'_id': _id}, {"$set": copy_data}, s_upsert=True)
